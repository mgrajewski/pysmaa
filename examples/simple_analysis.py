...
@author: Grajewski, FH Aachen University of Applied Sciences
"""
import sys
from os import path
import numpy as np
...
# provide the absolute path to pySMAA in the json-control-file and read it
# here.
if __name__ == '__main__':
    ...
    abs_path_to_pysmaa = parameter_dict['abs_path_to_pysmaa']

    # if an absolute path to pySMAA is provided, read it and append if, if not
    # already present in SYSTEMPATH
    if abs_path_to_pysmaa != '':
        if not any(path.normcase(sp) == abs_path_to_pysmaa for sp in sys.path):
            sys.path.append(f'{abs_path_to_pysmaa}/src')
...
def perform_simple_analysis(comp_pars_file):
    """
    This function is a wrapper for the actual computations of the weights from
    the rankings and the associated quantities.
    The names of the Excel input files (this can be more than one) are
    provided in `comp_pars_file`.
    The rankings (one-based!) are provided by an Excel file. The routine for
    reading the rankings expects an worksheet named 'Ranking' within this file.
    It is furthermore assumed that the names of the rankings are provided in
    the first row of the sheet and that the rankings are given as column
    vectors starting from the second row. The indices of the alternatives are
    meant to be sorted in descending order.
    The same Excel file must contain a worksheet named 'Characteristics' in
    which the performance matrix is stored. One column represents one criterion
    for all alternatives, such that the k-th row of the matrix represents all
    values of the k-th alternative. The first column and the first row are
    reserved for the names of the criteria and alternatives, such that the
    performance matrix must start in the second column and row in the Excel
    worksheet.
    This functions writes an Excel file with the results; its name is provided
    as well in `comp_pars_file`.

    Parameters
    ----------
    comp_pars_file : json-file
        It contains all necessary information for running the analyses. For
        details, we refer to the documentation provided in inoutput.py
    ...
    type_criterion = this_pars.promethee_pref_func_type*np.ones(Ptilde.shape[1], dtype=int)

    mcda_type = this_pars.mcda_type

    if mcda_type == 'Promethee':
        promethee_c = this_pars.promethee_c
        promethee_q = this_pars.promethee_q

    ncrit = Ptilde.shape[1]

    # create Excel workbook for the results of our computatio8n
    ...
    for iranking in range(0,len(rankings)):
        ...
        if mcda_type == 'Promethee':
            P = p_from_ptilde_prom_2(Ptilde, type_criterion, c=promethee_c, q=promethee_q)
        elif mcda_type == 'SAW':
        ...
        # analyse the W_r
        if this_pars.input_file_bounds != '':
            wel, Bel = inscribe_maximal_ellipsoid(P, current_ranking, \
                                                  A_constraints[iranking], \
        ...
        # consistency checks
        if wel.shape[0] >0:
            check = check_points(P, current_ranking, np.array([wel]))
            print('check w inside W_r:', check[0])

        if wel.shape[0] > 0:
            U, sizes = np.linalg.svd(Bel)[0:2]
            sizes = 2*sizes
        ...
        if wel.shape[0] >0:
            for i in range(0,wel.shape[0]):
                current_sheet.cell(row=5, column=i+2).value = wel[i]
        ...
        if wel.shape[0] > 0:
            ...
            # compute then. Moreover, we discard the last coordinate
            # (zero anyway).
