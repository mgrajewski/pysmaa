...
def read_ptilde_from_excel(file_name):
    ...
    There must be no empty columns in the part of the Excel sheet with the
    performance matrix.

    If both "Min" and "Max" exist, "Mean" is optional, if "Mean" exists, then
    both "Min" and "Max" are optional, but if one of these keywords exists, the
    other must be provided as well.

    The function returns the performance matrix Ptilde, its upper bounds
    PtildeMax and its lower bounds PtildeMin. It the bounds are not provided,
    it returns PtildeMax = PtildeMin = Ptilde. If mean values are not provided,
    it returns a matrix filled with zeroes for Ptilde.
    ...
    # this is the real number of criteria (could happen that there are some
    #filled Excel cells apart of the actual data such that max_column-1 is
    #greater than the number of criteria)
    ncrit = len(criteria)
    ...
    return Ptilde, PtildeMin, PtildeMax
...
def read_bounds_from_excel(file_name):
    """
    This function reads upper and lower bounds for weights from an Excel file
    for all stakeholders considered.
    It is assumed that the data are contained in a worksheet
    called bounds. This sheet title is case sensitive. The first row
    is reserved for the names of the stakeholders associated with the rankings,
    which are given column-wise in Excel.
    It is assumed in the Excel file, that any of the weights are bounded and
    that the order of weights corresponds to their order of the performance
    matrix. As the sum of weights is one, providing an upper bound of 1 and a
    lower bound of 0 corresponds to no constraints.
    For additional flexibility, upper and lower bounds are reformulated as
    linear conditions of the shape
        Aw <= b,
    w being the weight, as this format allows for arbitrary linear constraints
    like e.g. w_1 < w_2 in the future.
    ...
    All_A_constraints: list of 2D numpy arrays
        All_A_constraints[i] contains the constraints matrix for the i-th
        stakeholder
    All_b_constraints: list of 1D numpy arrays
        All_b_constraints[i] contains the constraints right hand side for the
        i-th stakeholder
    stakeholders : list of strings
        names of the stakeholders associated to the rankings
    """
