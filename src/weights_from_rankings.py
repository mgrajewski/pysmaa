...
def hrep_from_ranking(p: np.ndarray, ranking: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    """
    We assume a decision model m of the form :math:`m(w) = Pw, r = \\operatorname{argsort}(m(w))`.
    It is well known that a set of admissible weights leading to a given ranking :math:`r` is a convex polytope which
    can be expressed as :math:`{Aw \\leq b}`, the so-called H-representation. This function computes that
    H-representation from :math:`P` and the ranking :math:`r`. Any row in that systems represents one half space which
    may result from the actual ranking or the side conditions on the weights. :math:`amat` and :math:`b` are organized
    as follows:
    Let us denote the number of criteria by ncrit and the number of alternatives by nalts, and the number o
    alternatives represented in the ranking by nranks. At first, we code the ncrit conditions from the non-negativity
    of :math:`w`, then, nranks-1 conditions from the actual ranking, then nalts - nranks from all inferior alternatives
    and at last 2 from the normalization (sum of weights is at least one and at most one).
    ...
    -------
    amat : 2D-numpy array
        matrix amat representing the polytope due to :math:`Ax \\leq b`
    b : 1D-numpy array
    ...
    # compute the system of linear inequalities and store them in the matrix amat
    # number of total conditions: ncrit conditions from non-negativity, nalts-1 from the actual ranking, 2 from the
    # normalization (sum of weights is at least one and at most one)
    nconds = nalts - 1 + ncrit + 2
    amat = np.zeros((nconds, ncrit))
    b = np.zeros(nconds)
    ...
    # actual decision half spaces
    for i in range(nranks - 1):
        amat[i + ncrit, :] = p[ranking[i + 1], :] - p[ranking[i], :]
    ...
    if nranks < nalts:
        ...
        for i in range(nalts - nranks):
            amat[i + nranks + ncrit - 1, :] = p[alts_not_in_r[i], :] - p[ranking[-1], :]

    # normalization/being on the standard simplex
    amat[-2, :] = np.ones((1, ncrit))
    amat[-1, :] = -np.ones((1, ncrit))
...
def inscribe_maximal_ellipsoid(p: np.ndarray, ranking: np.ndarray, a_constraints: np.ndarray = None,
                               b_constraints: np.ndarray = None) -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    however to transform :math:`W_r` to :math:`R^{c-1}` first by :math:`x \\rightarrow qmat(x-cog)`, as :math:`W_r` is a
    ...
    Sources: Boyd, Stephen and Vandenberghe, Lieven: Convex Optimisation,
    ...
    amat, b = hrep_from_ranking(p, ranking)
    ...
    if (a_constraints is not None) and (b_constraints is not None):
        ...
        amat = np.vstack([a_constraints, amat])
    ...
    try:
        ...
        vertices = pp.duality.compute_polytope_vertices(amat, b)
    ...
    # bmat_init: downscaled unit matrix
    # w_init: mean of vertices transformed to R^{c-1}
    bmat_init = 0.01 * np.eye(ncrit - 1)
    w_init = (wmean - cog) @ qmat
    w_init = w_init[:-1]

    # this is the actual optimisation function
    w_opt, bmat_opt = fit_ellipsoid(ahat[:-2, :-1], bhat[:-2], w_init, bmat_init)

    # transform the midpoint w_opt back to the simplex
    w_opt = np.hstack([w_opt, [0]]) @ qmat.T + cog
    ...
    bmat_opt_aux = np.zeros((ncrit, ncrit))
    bmat_opt_aux[0:ncrit - 1, 0:ncrit - 1] = bmat_opt

    bmat_opt = qmat @ bmat_opt_aux
    return w_opt, bmat_opt


def verts_from_ranking(p: np.ndarray, ranking: np.ndarray, a_constraints: np.ndarray = None,
                       b_constraints: np.ndarray = None) -> np.ndarray:
    """
    ...
    nalts, ncrit, amat, b = __check_input(p, ranking, a_constraints, b_constraints)
...
def cheb_center_from_ranking(p: np.ndarray, ranking: np.ndarray, a_constraints: np.ndarray = None,
                             b_constraints: np.ndarray = None):
    """
    ...
    # Unfortunately, the polytope is degenerated, such that the Chebyshev-center does not make sense here (the product
    # of the distances to the hyperplanes is zero for ANY point in the polytope).
    # step one: shift the polytope such that it contained in the hyperplane <w,1> = 0 instead of <w,1> = 1
    ...
    # Chebyshev center
    # Delete the last two columns of the matrix, as these force the last coordinate to be zero. Instead, skip the last
    # coordinate entirely.
    try:
        ...
        cheb_center = qmat @ cheb_center + cog

    # catch exception that computing the Chebyshev center raises an exception
    except BaseException as exc:
...
def map_bmat_to_x(bmat: np.ndarray) -> np.ndarray:
    """
    ...
    Boyd, Stephen and Vandenberghe, Lieven: Convex Optimization, chapter 8.4.2, p. 414; Cambridge University Press.
...
def map_x_to_bmat(x: np.ndarray) -> np.ndarray:
    """
    This function is the counterpart to map_bmat_to_x.
...
def fit_ellipsoid(amat: np.ndarray, b: np.ndarray, w_init: np.ndarray, bmat_init: np.ndarray) \
        -> Tuple[np.ndarray, np.ndarray]:
    """
    This function computes the volume-maximal ellipsoid E inscribed into the
    polytope :math:`Ax \\leq b, E = \\{Bx + w \\mid ||x||_2 = 1 \\}`.
    Let the number of criteria be :math:`c`. Following Boyd, chapter 8.4.2, we describe an ellipsoid by

    .. math::

        E = \\{ Bx+w \\mid ||x||=1\\}

    with a symmetric :math:c x c -matrix and :math:`w` being the midpoint. The optimal ellipsoid described by
    :math:`B_{opt}` and :math:`w_{opt}` is given by the solution of the convex minimisation problem

    .. math::

        minimize -log(det(bmat)) s.t. ||Ba_i||_2 + \\langle a_i,w \\rangle \\leq  b_i

    We code the target function in the internal function fellipse and the constraints in fellipse_constr
    ...
    bmat_init : 2D-numpy-array
         starting value for :math:`bmat`

    Returns
    -------
    w_final : 1D-numpy-array
        center of the inscribed ellipsoid
    bmat_final : 2D-numpy-array
    ...
    def fellipse_constr(x):
        bmat = map_x_to_bmat(x)
        # not necessary, as ndim is known from outside
        # ndim = bmat.shape[0]
        bmat_times_amat_t = bmat @ amat.T
        norm_bmat_times_amat_t = np.sqrt(abs(np.diag(bmat_times_amat_t.T @ bmat_times_amat_t)))
        return -(norm_bmat_times_amat_t + amat @ x[0:ndim]) + b

    # this is how to code inequality constraints in scipy.optimize
    cons = {"type": "ineq", "fun": fellipse_constr}

    # get dimension from the shape of the starting value for bmat
    ndim = bmat_init.shape[0]

    # compute starting vector x_init from B_init and w_init (the first n entries in A belong to w)
    x_init = map_bmat_to_x(bmat_init)
    x_init[0:ndim] = w_init

    # actual minimization using scipy.optimize
    xmin = minimize(fellipse, x_init, constraints=cons, tol=1e-16)

    # reconstruct the optimal bmat and w from the optimal x
    bmat_final = map_x_to_bmat(xmin.x)
    ndim = bmat_final.shape[0]
    w_final = xmin.x[0:ndim]
    return w_final, bmat_final


def check_points(p: np.ndarray, ranking: np.ndarray, points: np.ndarray, a_constraints: np.ndarray = None,
                 b_constraints: np.ndarray = None) -> np.ndarray:
    """
    ...
    check_points : 1D-numpy-array
        an entry is true, if the corresponding point leads to ranking `ranking`
    """
    eps = -1e-12
...
def get_q(dim: int) -> np.ndarray:
    ...
    """
    # the columns of qmat contain the vertices of the standard simplex
    qmat = np.eye(dim)

    # We shift these vertices by the center of gravity. Then, the shifted
    # vertices belong to the hyperplane \sum w_i = 0. The rank of qmat is
    # dim-1 now, as the span of the columns of qmat is the hyperplane \sum w_i = 0.
    qmat = qmat - np.ones((1, dim)) / dim
    ...
    # k of qmat. Therefore, the columns of qmat but the last one provide the
    ...
    qmat = np.linalg.qr(qmat)[0]

    return qmat


def standard_simplex_to_flat(points: np.ndarray) -> np.ndarray:
    """
    This function transforms the points given in points on the standard simplex in :math:`R^{dim}` to essentially
    :math:`R^{dim-1}` using an orthogonal affine transformation.
    The counterpart of this function is flat_to_standard_simplex.

    Parameters
    ----------
    points : 2D-numpy-array
        points on the standard simplex to be transformed to the first dim-1 components of :math:`R^{dim}`.
...
def flat_to_standard_simplex(points: np.ndarray) -> np.ndarray:
    """
    ...
    # qmat is orthogonal, such that qmat^{-1} = qmat^T
    points = np.matmul(points, qmat.T)
...
def estimate_mu(points: np.ndarray):
    """
...
def bounding_box(vertices: np.ndarray) -> np.ndarray:
    """
    This function computes an axis-parallel bouding box of the convex hull of the vertices given in `vertices`.
    The function can be used as consistency check in finding representative weights from rankings.

    .. warning::
        As the bounding box is axis-parallel, the intervals the weights need to be contained in can be grossly
        overestimated.
    ...
    bounding box : 2D-numpy-array
        the first row contains the maxima for all coordinates, the second the minima
    """
...
def gaussian_on_simplex(mu: np.ndarray, sigma: np.ndarray, npoints: int) -> np.ndarray:
    """
    ...
    points : 2D-numpy-array
        ...
        around :math:`mu` with covariance matrix sigma
    """
...
def compute_margins(amat: np.ndarray, b: np.ndarray, points: np.ndarray) -> np.ndarray:
    """
    ...
    margins: 2D-numpy-array
        contains the absolute distances of the points to the hyperplanes of the polytope

    """
    # compute 1/|| a_k||
    aux = np.sqrt(1 / np.diag(amat @ amat.T))
    return aux * np.abs(b - amat @ points)


def check_p(p: np.ndarray):
    """
...
def __check_input(p: np.ndarray, ranking: np.ndarray, a_constraints: np.ndarray, b_constraints: np.ndarray) \
    ...
    return nalts, ncrit, amat, b


def main():
    # arbitrary raw performance matrix
    perfmat = np.array([[1, 2, 3], [2, 3, 1], [3, 2, 1]])
    benefit = np.ones(perfmat.shape[1], dtype=bool)

    # use SAW
    mcda_method = SawSum(benefit)

    # compute p from the actual performance matrix using the selected MCDA method (here: SAW, column-sum is one)
    p = mcda_method.p_from_perfmat(perfmat)

    # We provide a ranking and want to find its "center". The ranking refers to the 2nd alternative, as the ranking is
    # 0-based.
    ranking = np.array([1], dtype=int)

    # center and orientation matrix of the volume-maximal ellipsoid
    w_el, b_el = inscribe_maximal_ellipsoid(perfmat, ranking)

    # compute the H-representation of the polytope
    amat, b = hrep_from_ranking(p, ranking)

    # compute the margins, i.e. the distances to the facets of the polytope
    margins = compute_margins(amat, b, np.array(w_el))
    print('w_el: ' + str(w_el))
